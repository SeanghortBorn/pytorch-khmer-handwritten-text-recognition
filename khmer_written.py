...
num_class = 10
path = 'D:\PROJECTS\PYTORCH\data\kh_char_10_class/train/'
class Net(nn.Module):
    def __init__(self):
        super(Net, self).__init__()
        self.layer2 = nn.Linear(48*48, 64)
        self.layer3 = nn.Linear(64, 32)
        self.out_layer = nn.Linear(32, num_class)
...
train_set = DataSet(path, num_class)
...
for iter in range(500):
    ...
    real_b_sz = len(x)
    x = torch.tensor(x, dtype=torch.long) / 255  # tensor (b_sz, 28, 28)
