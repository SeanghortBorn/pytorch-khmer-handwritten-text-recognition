...
khmer_char = ['ខ', 'គ', 'ឃ', 'ង', 'ច', 'ឆ', 'ឈ', 'ដ', 'ណ', 'ត']
allX = list()
allY = list()
...
class DataSet(object):
    def __init__(self, path, num_class):
        ...
        for c in range(num_class):
            # c = khmer_char[num_class-1]
            tmp = listdir(path + khmer_char[c] + '/')
            n = int(len(tmp)*0.1)

            for file in tmp:

                img = cv2.imdecode(np.fromfile(path + khmer_char[c] + '/' + file, dtype=np.uint8), cv2.IMREAD_GRAYSCALE)
                x.append(img)  # np (H, W)
                y.append(c)
                allX.append(img)
                allY.append(c)
